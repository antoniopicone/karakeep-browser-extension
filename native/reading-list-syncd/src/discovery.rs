...
//! (commit 03eced8, src/discovery.rs) — dataset-agnostic (a peer is just an
//! address), so no changes needed for this single-dataset daemon. `NodeInfo`
//! /`ServiceInfo` below are unused here (main.rs defines its own single-
//! dataset `NodeInfo`) and kept only so this file stays a clean, diffable
//! copy of the original. Keep in sync by hand if serverless-sync's
//! discovery.rs ever changes; there's no build-time dependency on that repo
//! from here.
//!
