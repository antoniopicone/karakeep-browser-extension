//! Copied verbatim from github.com/antoniopicone/serverless-sync
//! (commit 03eced8, src/core.rs) — its own README explicitly documents
//! this file as the intended reuse point ("Copy core.rs into your crate
//! ... and rewrite the two functions where your actual data model lives").
//! This extension's entity/value model (a URL entity, a JSON-string value)
//! already fits the generic LWW register below with no changes needed, so
//! `apply`/`local_change` are untouched too. Keep this in sync by hand if
//! serverless-sync's core.rs ever changes; there's no build-time dependency
//! on that repo from here.
//!
...
impl Replica {
    ...
    pub fn apply(&mut self, op: Op) -> bool {
        ...
        // silently drop — real data loss. Verified by reproducing it before
        // this fix: restart, one local write, seq came back as 1 instead of
        // 3. `local_change` still does its own `self.seq += 1`, so this
